...
export class App extends Component {
  ...
  toggleEncryption = async () => {
    ...
    try {
      const decrypted = await window.nostr.nip04.decrypt(this.state.userPublicKey, fileContent);
      this.setState({ fileContent: decrypted });
    } catch (error) {
      const encrypted = await window.nostr.nip04.encrypt(this.state.userPublicKey, fileContent)
